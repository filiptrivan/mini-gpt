 ...
 *   2. Create a fresh tokenizer (BPE_BASE_VOCAB base byte tokens).
...
BPETokenizer *bpe_load(const char *filepath) {
    ...
    /* Start with a fresh tokenizer (base bytes only, no merges yet). */
    ...
    tok->vocab = realloc(tok->vocab,
                         (size_t)(BPE_BASE_VOCAB + n) * sizeof(VocabEntry));
    ...
    /* Reconstruct each merged vocab entry by replaying the merge. Because
     * merges reference earlier ids only (training enforces this priority
     * order), iterating i from 0 upwards always has both parents present. */
    for (int i = 0; i < n; i++) {
        bpe_build_merged_vocab_entry(tok->vocab, BPE_BASE_VOCAB + i,
                                     tok->merges[i].a, tok->merges[i].b);
    }

    /* Finalize bookkeeping only after the vocab loop fully succeeds, so
     * a failure earlier leaves bpe_free's loop bound (vocab_size) at the
     * base size — which avoids touching uninitialized vocab entries. */
    tok->num_merges = n;
    tok->vocab_size = BPE_BASE_VOCAB + n;
